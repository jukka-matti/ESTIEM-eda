...
from ..core.calculations import calculate_i_chart, calculate_capability, calculate_probability_plot
...
class ProcessAnalysisTool(SimplifiedMCPTool):
    ...
    def analyze(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            results["stability_analysis"] = {
                ...
                "control_status": "in_control" if len(stability_results.get("out_of_control_indices", [])) == 0 else "out_of_control"
            }
        ...
        if spec_limits:
            try:
                capability_results = calculate_capability(values, spec_limits, f"{title} - Capability Assessment")
                results["capability_analysis"] = {
                    "type": "capability",
                    "statistics": self.format_statistics(capability_results.get("statistics", {})),
                    "capability_indices": self.format_statistics(capability_results.get("capability_indices", {})),
                    "defect_analysis": self.format_statistics(capability_results.get("defect_analysis", {})),
        ...
        try:
            distribution_results = calculate_probability_plot(values, distribution, confidence_level, f"{title} - Distribution Assessment")
            results["distribution_analysis"] = {
                ...
                "goodness_of_fit": self.format_statistics(distribution_results.get("goodness_of_fit", {})),
    ...
    def create_comprehensive_interpretation(self, results: Dict[str, Any]) -> str:
        ...
        interpretations.append(f"Process analysis of {sample_size} measurements with mean value {mean_val:.4f}.")
    ...
    def create_overall_assessment(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if control_status == "out_of_control":
            assessment["recommendations"].append("Investigate and eliminate sources of special cause variation")
        ...
        if "capability_indices" in capability:
            ...
            if cpk >= 1.33:
                ...
                assessment["recommendations"].append("Improve process capability through variation reduction")
            else:
                assessment["capability_status"] = "not_capable"
                assessment["recommendations"].append("Significant process improvement required to meet specifications")
        ...
        if "goodness_of_fit" in distribution:
            ...
            if p_value > 0.05:
                ...
                assessment["recommendations"].append("Consider alternative distribution or data transformation")
        
        # Overall status determination
        if (assessment["stability_status"] == "in_control" and 
            assessment["capability_status"] == "capable" and
            assessment["distribution_status"] == "fits_assumed"):
            assessment["overall_status"] = "excellent"
        elif (assessment["stability_status"] == "in_control" and 
              assessment["capability_status"] in ["capable", "marginal"]):
