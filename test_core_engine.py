...
def test_core_calculations():
    ...
    try:
        ...
        print(f"   ✅ I-Chart: Mean={result['statistics']['mean']:.3f}, UCL={result['statistics']['ucl']:.3f}")
        
        # Test 2: Process Capability
        print("\n2. Testing Process Capability...")
        result = calculate_process_capability(values, lsl=9.0, usl=11.0)
        print(f"   ✅ Capability: Cp={result['capability_indices']['cp']:.3f}, Cpk={result['capability_indices']['cpk']:.3f}")
        ...
        print(f"   ✅ ANOVA: F={result['anova_results']['f_statistic']:.3f}, p={result['anova_results']['p_value']:.3f}")
        
        # Test 4: Pareto
        print("\n4. Testing Pareto Analysis...")
        pareto_data = {"Issue_A": 100, "Issue_B": 50, "Issue_C": 25, "Issue_D": 15}
        result = calculate_pareto(pareto_data)
        print(f"   ✅ Pareto: {len(result['vital_few']['categories'])} vital few categories at {result['vital_few']['percentage']:.1f}%")
        
        # Test 5: Probability Plot
        print("\n5. Testing Probability Plot...")
        result = calculate_probability_plot(values, distribution='normal')
        print(f"   ✅ Probability Plot: r={result['goodness_of_fit']['correlation_coefficient']:.4f} ({result['goodness_of_fit']['interpretation']})")
...
def test_mcp_tools():
    ...
    try:
        ...
        if result['success']:
            print("   ✅ I-Chart MCP tool working")
        else:
            print(f"   ❌ I-Chart failed: {result.get('error')}")
            
        # Test Capability Tool
        from estiem_eda.tools.capability import CapabilityTool
        tool = CapabilityTool()
        result = tool.execute({"data": [10.0, 10.5, 9.5, 10.2, 9.8], "lsl": 9.0, "usl": 11.0})
        if result['success']:
            print("   ✅ Capability MCP tool working")
        else:
            print(f"   ❌ Capability failed: {result.get('error')}")
